...
def build_words_context_prompt(selected_cards, distractor_cards, text_level):
    ...
    lines.append("Это должна быть маленькая история, сцена из жизни, диалоговая ситуация или небольшой рассказ, а не набор слабо связанных предложений.")
    lines.append("")
    ...
    lines.append("- Пользователь должен тренировать слова в живом контексте.")
    lines.append("- Текст должен иметь понятный сюжет, настроение и внутреннюю логику.")
    lines.append("- Предложения должны быть связаны между собой: события, персонажи и детали должны продолжаться от предложения к предложению.")
    lines.append("- Не делай отдельные изолированные предложения только ради вставки слов.")
    lines.append("- Текст должен звучать как хороший учебный мини-текст из учебника немецкого языка.")
    lines.append("")
    lines.append("Длина текста:")
    lines.append("- Не ограничивайся одним предложением на слово.")
    lines.append("- Напиши столько предложений, сколько нужно, чтобы получился естественный и интересный текст.")
    lines.append("- Текст может быть длиннее, если это помогает сделать историю связной.")
    lines.append("- Не растягивай текст искусственно, но не делай его слишком коротким и сухим.")
    lines.append("")
    ...
    lines.append("- Если уровень A1–A2, используй короткие, понятные предложения и простую лексику.")
    lines.append("- Если уровень B1–B2, можно делать текст более насыщенным и естественным.")
    lines.append("- Можно использовать маленький конфликт, цель, неожиданную ситуацию, план на день, поездку, учёбу, работу, знакомство, проблему или короткий диалог.")
    lines.append("- У текста должно быть начало, развитие и небольшой финал.")
    lines.append("- Не пиши механический список предложений вида: 'Он пошёл туда. Потом сделал это. Потом сделал то.'")
    lines.append("- Не выделяй слова жирным.")
    lines.append("- Не используй markdown.")
    lines.append("")
    lines.append("Задача:")
    lines.append("- Используй все MAIN WORDS в общем немецком тексте.")
    ...
    lines.append("- Каждый пропуск должен находиться внутри фрагмента текста, а не в отдельном изолированном предложении.")
    lines.append("- Для поля blank_text используй фрагмент из 2–3 связанных предложений из общего текста.")
    lines.append("- В blank_text одно из слов или выражений должно быть заменено на ____.")
    lines.append("- В каждом blank_text должен быть только один пропуск ____.")
    lines.append("- Остальные предложения во фрагменте должны давать смысловой контекст.")
    lines.append("- blank_text должен быть взят из full_text, но с заменой правильного ответа на ____.")
    lines.append("")
    lines.append("Правило для вариантов ответа:")
    ...
    lines.append("- Остальные 3 варианта должны быть неправильными по смыслу именно для этого контекста.")
    ...
    lines.append("- Делай фрагмент достаточно конкретным, чтобы правильный ответ был однозначен.")
    lines.append("- Если нужно, добавь больше контекстных деталей в blank_text, чтобы исключить другие варианты.")
    ...
    lines.append("- В каждом задании должен быть только один 100% правильный вариант.")
    lines.append("- Остальные варианты могут быть грамматически похожими, но должны быть неправильными по смыслу.")
    lines.append("- Не создавай фрагменты, где несколько вариантов звучат естественно.")
    lines.append("- Если несколько вариантов могут подойти, добавь больше контекста в blank_text.")
    lines.append("- Контекст должен помогать выбрать правильное слово, а не заставлять угадывать.")
    ...
    lines.append("- blank_text: 'Lukas hat heute Morgen gemerkt, dass er kein Bargeld mehr hat. Er braucht Geld für die Reise nach Berlin. Deshalb muss er zuerst zu ____ gehen.'")
    lines.append("- correct_option: 'der Bank'")
    lines.append("- Здесь 'der Bank' — единственный правильный вариант, потому что контекст 'Bargeld' и 'Geld' делает ответ однозначным.")
    lines.append("")
    lines.append("Пример плохого задания:")
    lines.append("- blank_text: 'Zuerst muss er zu ____ gehen, um eine wichtige Sache zu erledigen.'")
    ...
    lines.append('      "sentence": "полное предложение без пропуска, где использовано нужное слово",')
    lines.append('      "blank_text": "фрагмент из 2–3 связанных предложений из общего текста, где нужное слово заменено на ____",')
    ...
    lines.append("}")
    lines.append("")
    lines.append("Важно для JSON:")
    lines.append("- full_text должен быть цельным связным текстом.")
    lines.append("- summary должен быть коротким summary на русском языке: о чём был текст и какие слова потренированы.")
    lines.append("- В items должен быть один item для каждого MAIN WORD.")
    lines.append("- Каждый item должен использовать card_id соответствующего MAIN WORD.")
    lines.append("- sentence — это полное предложение из текста без пропуска.")
    lines.append("- blank_text — это не одно короткое предложение, а фрагмент из 2–3 связанных предложений.")
    lines.append("- blank_text должен содержать ровно один пропуск ____.")
